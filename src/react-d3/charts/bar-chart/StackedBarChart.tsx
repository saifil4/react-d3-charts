...
import { chartData } from "react-d3/data/stacked-bar-chart-data";
                ...
                </Axis>
                {
                    data.map((d) => (
                        <g>
                            ...
                            <motion.rect
                                initial={{ height: 0, y: height }}
                                animate={{ height: height - yAxisScale(d.param2), y: yAxisScale(d.param2 + d.param1) }}
                                transition={{ duration: 1 }}
                                x={xAxisScale(d.name)}
                                width={xAxisScale.bandwidth()} fill="red" />
                            <motion.rect
                                initial={{ height: 0, y: height }}
                                animate={{ height: height - yAxisScale(d.param3), y: yAxisScale(d.param3+ d.param2 + d.param1) }}
                                transition={{ duration: 1 }}
                                x={xAxisScale(d.name)}
                                width={xAxisScale.bandwidth()} fill="blue" />
                        </g>
                    ))
                }
            </g>
